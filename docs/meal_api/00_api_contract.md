...
| GET | `/api/v1/meals/my/items/` (`/api/meals/my/items/`) | `IsAuthenticated` + 활성 사용자 | Query: `ym`(선택) | `ym`, `items[]` (`id`, `used_date`, `merchant_name`, `approval_no`, `total_amount`, `my_amount`, `participants_count`, `participants_sum`, `created_by`, `can_edit`, `can_delete`) | 200, 400, 401, 403 | 페이지네이션 없음, 정렬=`used_date ASC, id ASC`, 필터=`ym` + 참여자 본인 |
| GET | `/api/v1/meals/my/created/` (`/api/meals/my/created/`) | `IsAuthenticated` + 활성 사용자 | Query: `ym`(선택, `YYYYMM`/`YYYY-MM`, 미입력 시 현재월) | `ym`, `items[]` (`id`, `used_date`, `merchant_name`, `approval_no`, `total_amount`, `my_amount`, `participants_count`, `participants_sum`, `created_by`, `can_edit`, `can_delete`) | 200, 400, 401, 403 | 페이지네이션 없음, 정렬=`used_date ASC, id ASC`, 필터=`ym` + 작성자 본인 |
...
| GET | `/api/v1/meals/claims/{claim_id}/` (`/api/meals/claims/{claim_id}/`) | `IsAuthenticated` + 활성 사용자 + (해당 claim 참여자 또는 작성자) | Path: `claim_id` | claim 상세(POST 응답과 동일 구조)                                                                                                                                                           | 200, 401, 403, 404 | 페이지네이션/정렬 없음 |
...
### 3) GET my created
- Query
  - `ym`: string, optional (`svc.normalize_ym` 기준, 미입력 시 현재월)
- 필터/정렬
  - branch: `request.branch` 우선, 없으면 `request.user.branch`
  - 작성자 기준: `MealClaim.user == request.user`
  - `is_deleted = false`
  - `used_date` 월 범위 필터
  - `used_date ASC, id ASC`
- Response 200
  - `ym`: string
  - `items`: array (구조는 `GET my items`와 동일)
  - `my_amount`: 생성자가 참여자에 없으면 `0`
...
- 상세 조회(GET detail)는 참여자 또는 작성자 가능, 수정/삭제는 작성자만 가능.
...
- Ordering:
  ...
  - `GET my/created`: `used_date ASC, id ASC`
- Filters:
  - `summary`, `my/items`, `my/created`: `ym`
...
- `mysite/api/v1/views/meals.py:122` (`MealMyCreatedAPIView`)
- `mysite/api/v1/views/meals.py:176` (`MealOptionsAPIView`)
- `mysite/api/v1/views/meals.py:232` (`MealClaimCreateAPIView`)
- `mysite/api/v1/views/meals.py:261` (`MealClaimDetailAPIView`)
